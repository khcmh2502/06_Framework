...
VALUES(1, 1994); -- 1번 회원이 1994번 게시글에 좋아요를 누름
...
-- 작성일 : 몇 초/몇 분/몇 시간 전 조회, 하루가 넘어가면 YYYY-MM-DD 형식 조회
...
(SELECT COUNT(*)
FROM "BOARD_LIKE"
WHERE BOARD_NO = 1994) LIKE_COUNT,
 ...
 * 
 * 특정 사용자가 참여한 채팅방 목록, 각 채팅방의 최근 메시지, 관련된 정보 등 조회 목적
 * */
SELECT CHATTING_ROOM_NO
	...
	,NVL2((SELECT OPEN_MEMBER FROM CHATTING_ROOM R2
		...
		R.PARTICIPANT, -- NULL이 아닌경우 (로그인한사람 오픈멤버일때는 참가자 타겟번호)
		R.OPEN_MEMBER	 -- NULL 인 경우 (내가 오픈멤버가 아닐때는 OPEN_MEMBER가 타켓번호)
		) TARGET_NO	
	,NVL2((SELECT OPEN_MEMBER FROM CHATTING_ROOM R2
		WHERE R2.CHATTING_ROOM_NO = R.CHATTING_ROOM_NO
		AND R2.OPEN_MEMBER = 1),
...
DELETE FROM REFRESH_TOKEN WHERE MEMBER_NO = 1;
COMMIT;
...
COMMENT ON COLUMN "REFRESH_TOKEN"."MEMBER_NO" IS '회원번호(PFK)';

COMMENT ON COLUMN "REFRESH_TOKEN"."REFRESH_TOKEN" IS '리프레시토큰';

COMMENT ON COLUMN "REFRESH_TOKEN"."EXPIRATION_DATE" IS '만료일';

ALTER TABLE "REFRESH_TOKEN" ADD CONSTRAINT "FK_MEMBER_TO_REFRESH_TOKEN_1" FOREIGN KEY (
	"MEMBER_NO"
)
REFERENCES "MEMBER" (
	"MEMBER_NO"
